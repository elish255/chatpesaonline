...
import { createFimiPayment, checkFimiPayment } from "@/lib/fimipay.functions";
...
function Lipa() {
  ...
  const createPayment = useServerFn(createFimiPayment);
  const checkPayment = useServerFn(checkFimiPayment);
  const manualPayment = useServerFn(submitManualPayment);
  const me = useServerFn(getMe);
  const [phone, setPhone] = useState("");
  const [method, setMethod] = useState<"fimipay" | "lipa_namba">("fimipay");
  ...
  useEffect(() => {
    ...
    void me().then((res) => { if (res.user?.status === "active") void navigate({ to: "/dashboard" }); if (res.user?.phone && !r?.phone) setPhone(String(res.user.phone)); }).catch(() => undefined);
  }, [me]);
  ...
  async function startFimiPay(e: React.FormEvent) {
          ...
          {method === "fimipay" ? (
            ...
            <div className="mt-5 space-y-4">
              ...
              <div className="space-y-2">{[
                ["Vodacom M-Pesa", "*150*00# → Lipa kwa M-PESA → Lipa kwa simu → weka 251161660 → 16,000"],
                ["Mixx by Yas", "*150*01# → Lipa kwa simu → mitandao mingine → HaloPesa → 251161660 → 16,000"],
                ["Airtel Money", "*150*60# → Lipia Bili → Lipa kwa simu → HaloPesa → kumbukumbu 251161660 → 16,000"],
                ["HaloPesa", "*150*88# → Lipia Bidhaa → HaloPesa → 251161660 → 16,000"],
