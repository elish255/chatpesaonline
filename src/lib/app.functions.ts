...
export const registerUser = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const existing = await supabaseRest<Array<Record<string, unknown>>>("chatpesa_users", {
    ...
    const created = await supabaseRest<Array<Record<string, unknown>>>("chatpesa_users", {
      method: "POST",
      body: {
        name: data.name,
        username,
...
export const getNotifications = createServerFn({ method: "GET" }).handler(async () => {
  const userId = await requireUser();
  const rows = await supabaseRest<Array<Record<string, unknown>>>("chatpesa_notifications", {
...
export const completeChat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const users = await supabaseRest<Array<Record<string, unknown>>>("chatpesa_users", { query: { select: "status", id: `eq.${userId}`, limit: 1 } });
    if (users[0]?.status !== "active") throw new Error("ACCOUNT_NOT_ACTIVE");
    const existing = await supabaseRest<Array<Record<string, unknown>>>("chatpesa_chat_sessions", { query: { select: "id", session_id: `eq.${data.sessionId}`, limit: 1 } });
    if (existing.length) return { ok: true, alreadyCompleted: true };
    await supabaseRest("chatpesa_chat_sessions", { method: "POST", body: { session_id: data.sessionId, user_id: userId, person_name: data.personName, payout, message_count: 20, status: "completed", completed_at: new Date().toISOString() } });
    const current = await supabaseRest<Array<Record<string, unknown>>>("chatpesa_users", { query: { select: "balance", id: `eq.${userId}`, limit: 1 } });
    const balance = Number(current[0]?.balance ?? 0);
    await supabaseRest("chatpesa_users", { method: "PATCH", query: { id: `eq.${userId}` }, body: { balance: balance + payout } });
    await supabaseRest("chatpesa_notifications", { method: "POST", body: { user_id: userId, title: "Umejipatia malipo 💰", message: `Chat na ${data.personName} imekamilika. TZS ${payout.toLocaleString()} imeongezwa kwenye salio lako.`, type: "success" } });
...
export const requestWithdrawal = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await supabaseRest("chatpesa_withdrawals", {
      method: "POST",
...
export const getAdminData = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const [users, payments, withdrawals, notifications] = await Promise.all([
    supabaseRest<Array<Record<string, unknown>>>("chatpesa_users", { query: { select: "id,name,username,email,phone,status,balance,withdrawn,created_at", order: "created_at.desc", limit: 200 } }),
    supabaseRest<Array<Record<string, unknown>>>("chatpesa_activation_payments", { query: { select: "id,user_id,method,amount,phone,status,created_at,external_id", order: "created_at.desc", limit: 200 } }),
    supabaseRest<Array<Record<string, unknown>>>("chatpesa_withdrawals", { query: { select: "id,user_id,amount,method,account_number,status,created_at", order: "created_at.desc", limit: 200 } }),
    supabaseRest<Array<Record<string, unknown>>>("chatpesa_notifications", { query: { select: "id,user_id,title,message,type,created_at", order: "created_at.desc", limit: 50 } }),
  ]);
  return { users, payments, withdrawals, notifications };
});

export const adminSetUserStatus = createServerFn({ method: "POST" })
  .validator(z.object({ userId: z.string().uuid(), status: z.enum(["active", "rejected", "pending"]) }))
  .handler(async ({ data }) => {
    await requireAdmin();
    await supabaseRest("chatpesa_users", { method: "PATCH", query: { id: `eq.${data.userId}` }, body: { status: data.status, ...(data.status === "active" ? { activated_at: new Date().toISOString() } : {}) } });
    await supabaseRest("chatpesa_notifications", {
      method: "POST",
      body: {
        user_id: data.userId,
        title: data.status === "active" ? "Akaunti imewashwa 🎉" : data.status === "rejected" ? "Akaunti imekataliwa" : "Akaunti imerudishwa pending",
        message: data.status === "active" ? "Akaunti yako iko active. Unaweza kuingia Dashboard na kuanza kuchat." : data.status === "rejected" ? "Akaunti yako imewekwa rejected. Tafadhali wasiliana na support." : "Akaunti yako imerudishwa kwenye pending.",
...
export const adminReviewPayment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await supabaseRest("chatpesa_activation_payments", { method: "PATCH", query: { id: `eq.${data.paymentId}` }, body: { status, reviewed_at: new Date().toISOString() } });
    const userId = String(payment.user_id);
    if (data.action === "approve") {
      await supabaseRest("chatpesa_users", { method: "PATCH", query: { id: `eq.${userId}` }, body: { status: "active", activated_at: new Date().toISOString() } });
      await supabaseRest("chatpesa_notifications", { method: "POST", body: { user_id: userId, title: "Akaunti ime-activate 🎉", message: "Malipo yako yamehakikiwa. Ingia dashboard kuanza kuchat.", type: "success" } });
    } else {
      await supabaseRest("chatpesa_notifications", { method: "POST", body: { user_id: userId, title: "Malipo yamekataliwa", message: "Admin hakuthibitisha malipo yako. Tafadhali wasiliana na support na uthibitishe taarifa za muamala.", type: "error" } });
    }
    return { ok: true };
  });
...
export const adminReviewWithdrawal = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    await requireAdmin();
    const rows = await supabaseRest<Array<Record<string, unknown>>>("chatpesa_withdrawals", { query: { select: "id,user_id,amount,status", id: `eq.${data.withdrawalId}`, limit: 1 } });
    const item = rows[0];
    ...
    if (data.action === "approve") {
      const users = await supabaseRest<Array<Record<string, unknown>>>("chatpesa_users", { query: { select: "balance,withdrawn", id: `eq.${userId}`, limit: 1 } });
      const balance = Number(users[0]?.balance ?? 0);
      if (balance < amount) throw new Error("INSUFFICIENT_BALANCE");
      await supabaseRest("chatpesa_users", { method: "PATCH", query: { id: `eq.${userId}` }, body: { balance: balance - amount, withdrawn: Number(users[0]?.withdrawn ?? 0) + amount } });
      await supabaseRest("chatpesa_withdrawals", { method: "PATCH", query: { id: `eq.${data.withdrawalId}` }, body: { status: "approved", reviewed_at: new Date().toISOString() } });
      await supabaseRest("chatpesa_notifications", { method: "POST", body: { user_id: userId, title: "Withdrawal imeidhinishwa", message: `Withdrawal ya TZS ${amount.toLocaleString()} imeidhinishwa na admin.`, type: "success" } });
    } else {
      await supabaseRest("chatpesa_withdrawals", { method: "PATCH", query: { id: `eq.${data.withdrawalId}` }, body: { status: "rejected", reviewed_at: new Date().toISOString() } });
      await supabaseRest("chatpesa_notifications", { method: "POST", body: { user_id: userId, title: "Withdrawal imekataliwa", message: "Admin amekataa ombi lako la withdrawal. Wasiliana na support kwa maelezo zaidi.", type: "error" } });
    }
    return { ok: true };
  });
...
export const adminSendNotification = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    await requireAdmin();
    await supabaseRest("chatpesa_notifications", { method: "POST", body: { user_id: data.userId, title: data.title, message: data.message, type: data.type } });
    return { ok: true };
  });
